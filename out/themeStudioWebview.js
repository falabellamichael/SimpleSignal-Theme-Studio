...
class ThemeStudioWebview {
                ...
                switch (message.command) {
                    ...
                    case 'saveProfile':
                        if (message.name && message.colors) {
                            await profileManager_1.ProfileManager.saveProfile(message.name, message.colors, message.tokenColors || [], message.type || 'dark');
                    ...
                    case 'deleteProfile':
                        if (message.id) {
                            await profileManager_1.ProfileManager.deleteProfile(message.id);
                    ...
                    case 'copyJson':
                        if (message.colors) {
                            const jsonStr = themeEngine_1.ThemeEngine.exportAsSettingsJson(message.colors, message.tokenColors);
    ...
    _getHtmlForWebview(webview) {
        ...
        const activeProfileName = vscode.workspace.getConfiguration('simpletheme').get('activeProfile', 'Cyberpunk Neon');
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <script>
    (function() {
      ...
      function buildTokenRules() {
        const rules = [];
        const scopeDefs = ${JSON.stringify(presets_1.SYNTAX_SCOPE_DEFINITIONS)};
