...
class ThemeStudioWebview {
    ...
    _internalChangeDepth = 0;
    _internalChangeTimer;
    _syncRequestedDuringInternalChange = false;
    ...
    async _runInternalChange(operation) {
                ...
                switch (message.command) {
                    case 'applyAll':
                        const appliedState = await this._enqueueMutation(async () => {
                            await this._runInternalChange(() => themeEngine_1.ThemeEngine.applyTheme(message.colors, message.tokenColors, message.profileName));
                            return themeEngine_1.ThemeEngine.getEffectiveThemeState();
                        });
                    ...
                    case 'applyLiveColors': {
                        ...
                        const applyTask = this._enqueueMutation(async () => {
                            await this._runInternalChange(() => themeEngine_1.ThemeEngine.applyColors(colors));
                        });
                        let errorMessage;
                        try {
                            await applyTask;
                    ...
                    case 'applyLiveTokenColors': {
                        const colors = message.colors && typeof message.colors === 'object' ? message.colors : {};
                        const applyTask = this._enqueueMutation(async () => {
                            await this._runInternalChange(() => themeEngine_1.ThemeEngine.applyTokenColors(colors));
                        });
                        let errorMessage;
                        try {
                            await applyTask;
                    ...
                    case 'applyPreset':
                        ...
                        if (preset) {
                            const presetState = await this._enqueueMutation(async () => {
                                await this._runInternalChange(() => themeEngine_1.ThemeEngine.applyPreset(preset));
                                return themeEngine_1.ThemeEngine.getEffectiveThemeState();
                            });
                    ...
                    case 'saveProfile':
                        ...
                        if (name && name.trim()) {
                            await profileManager_1.ProfileManager.saveProfile(name.trim(), message.colors, message.tokenColors, message.type === 'light' ? 'light' : 'dark');
                    ...
                    case 'loadProfile':
                        ...
                        if (profile) {
                            const profileState = await this._enqueueMutation(async () => {
                                await this._runInternalChange(() => themeEngine_1.ThemeEngine.applyTheme(profile.colors, profile.tokenColors, profile.name, profile.type));
                                return themeEngine_1.ThemeEngine.getEffectiveThemeState();
                            });
                    ...
                    case 'refreshThemeFromVsCode': {
                        const activeStateObj = await this._enqueueMutation(async () => themeEngine_1.ThemeEngine.getEffectiveThemeState());
                    ...
                    case 'resetTheme': {
                        ...
                        const resetResult = await this._enqueueMutation(async () => {
                            ...
                            if (confirmed) {
                                await this._runInternalChange(() => themeEngine_1.ThemeEngine.resetTheme());
                            }
                            return {
                                confirmed,
                                state: themeEngine_1.ThemeEngine.getEffectiveThemeState(),
                            };
                        });
                        ...
                        this._panel.webview.postMessage({
                            ...
                            liveApply: vscode.workspace.getConfiguration('simpletheme').get('liveApply', true),
                            requestRevision: message.requestRevision,
                        });
                        break;
                    }
    ...
    _syncActiveThemeToWebview(requestRevision, effective = themeEngine_1.ThemeEngine.getEffectiveThemeState()) {
    ...
    _getHtmlForWebview() {
        const effectiveState = themeEngine_1.ThemeEngine.getEffectiveThemeState();
        ...
        const liveApplyEnabled = vscode.workspace.getConfiguration('simpletheme').get('liveApply', true);
        ...
        const uiDefaults = Object.fromEntries(presets_1.UI_COLOR_DEFINITIONS.map((definition) => [definition.id, definition.defaultValue]));
        const normalizeComparableColor = (value) => value.trim().toLowerCase();
        const getSimpleUiGroupState = (definition) => {
        ...
        const getSyntaxColor = (syntaxId) => {
            const syntax = presets_1.SYNTAX_SCOPE_DEFINITIONS.find((definition) => definition.id === syntaxId);
            if (!syntax)
                return undefined;
        ...
        const getSimpleSyntaxGroupState = (definition) => {
            const values = definition.targets.map((target) => getSyntaxColor(target) || presets_1.SYNTAX_SCOPE_DEFINITIONS.find((syntax) => syntax.id === target)?.defaultColor || definition.defaultColor);
            return {
                displayValue: values[0] || definition.defaultColor,
                mixed: new Set(values.map(normalizeComparableColor)).size > 1,
            };
        };
        const renderSimpleUiCard = (definition) => {
        ...
        const renderSimpleSyntaxCard = (definition) => {
        ...
        const renderSimpleSections = (definitions, renderCard) => Array.from(new Set(definitions.map((definition) => definition.section))).map((section) => {
        ...
        const escapeHtml = (value) => value
            ...
            .replace(/"/g, '&quot;')
            .replace(/'/g, '&#39;');
        const simpleUiOwnerByTarget = new Map(presets_1.SIMPLE_UI_DEFINITIONS.flatMap((definition) => definition.targets.map((target) => [target, definition.id])));
        const simpleSyntaxOwnerByTarget = new Map(presets_1.SIMPLE_SYNTAX_DEFINITIONS.flatMap((definition) => definition.targets.map((target) => [target, definition.id])));
        const requireSimpleOwner = (owners, target, label) => {
            const owner = owners.get(target);
            if (!owner)
                throw new Error(`${label} preview target ${target} has no Simple owner`);
            return owner;
        };
        const previewRoleKind = (id) => {
        ...
        const renderPreviewUiOption = (definition) => {
        ...
        const renderPreviewUiGallery = () => Array.from(new Set(presets_1.UI_COLOR_DEFINITIONS.map((definition) => definition.category))).map((category) => {
            const categoryDefinitions = presets_1.UI_COLOR_DEFINITIONS.filter((definition) => definition.category === category);
        ...
        const renderPreviewSyntaxOption = (definition) => {
...
<body>
    ...
    <div class="main-column">
      ...
      <div id="tab-ui" class="tab-pane">
        ...
        <div id="uiSimpleContainer">
          <div style="font-size: 11px; color: var(--text-muted); margin-bottom: 12px;">
            ${presets_1.SIMPLE_UI_DEFINITIONS.length} linked controls cover all ${presets_1.UI_COLOR_DEFINITIONS.length} color roles used by every bundled preset. A <strong>Mixed</strong> badge means those linked roles currently differ; choosing a color intentionally unifies them. <em>Tip: click a preview element to jump to its control.</em>
          </div>
          ${renderSimpleSections(presets_1.SIMPLE_UI_DEFINITIONS, renderSimpleUiCard)}
        </div>
      ...
      <div id="tab-syntax" class="tab-pane" style="display: none;">
        ...
        <div class="mode-switch-bar">
          ...
          <div class="mode-toggle-group">
            <button class="mode-btn active" id="btnSyntaxModeSimple" onclick="setSyntaxMode('simple')">⚡ Simple Mode (${presets_1.SIMPLE_SYNTAX_DEFINITIONS.length} Colors)</button>
            <button class="mode-btn" id="btnSyntaxModeAdvanced" onclick="setSyntaxMode('advanced')">⚙️ Advanced Mode (Full Scopes)</button>
          </div>
        </div>

        <!-- Tab 2A: Simple Mode Syntax (${presets_1.SIMPLE_SYNTAX_DEFINITIONS.length} Master Colors) -->
        <div id="syntaxSimpleContainer">
          <div style="font-size: 11px; color: var(--text-muted); margin-bottom: 12px;">
            ${presets_1.SIMPLE_SYNTAX_DEFINITIONS.length} linked controls cover all ${presets_1.SYNTAX_SCOPE_DEFINITIONS.length} supported syntax roles. A <strong>Mixed</strong> badge preserves intentional differences until you choose one shared color. <em>Tip: click any code token in the preview to jump to its control.</em>
          </div>
          ${renderSimpleSections(presets_1.SIMPLE_SYNTAX_DEFINITIONS, renderSimpleSyntaxCard)}
        </div>
    ...
    <div class="preview-column" id="previewColumn">
      <div class="preview-sticky">
        ...
        <div class="preview-header-bar">
          ...
          <div class="preview-controls-row">
            ...
            <div class="size-btn-group" role="group" aria-label="Preview content">
              ...
              <button type="button" class="size-btn preview-mode-btn" data-preview-mode="ui" aria-pressed="false">All UI (${presets_1.UI_COLOR_DEFINITIONS.length})</button>
              <button type="button" class="size-btn preview-mode-btn" data-preview-mode="syntax" aria-pressed="false">Syntax (${presets_1.SYNTAX_SCOPE_DEFINITIONS.length})</button>
            </div>
        ...
        <div class="preview-panel preview-role-gallery" id="previewUiPanel" data-preview-panel="ui" hidden>
          <p class="preview-gallery-intro">All ${presets_1.UI_COLOR_DEFINITIONS.length} granular UI options are shown here. Click any sample or label to open its exact Advanced control, or its owning Simple control when Simple Mode is active.</p>
        ...
        <div class="preview-panel preview-role-gallery" id="previewSyntaxPanel" data-preview-panel="syntax" hidden>
          <p class="preview-gallery-intro">All ${presets_1.SYNTAX_SCOPE_DEFINITIONS.length} syntax options are independently clickable. Grouped Simple controls still keep every repeated token reachable from the preview.</p>
          <section class="preview-option-section" aria-label="Syntax colors">
            <div class="preview-option-grid">${presets_1.SYNTAX_SCOPE_DEFINITIONS.map(renderPreviewSyntaxOption).join('')}</div>
          </section>
        </div>
  ...
  <script>
    ...
    (function() {
      ...
      const ALL_PRESETS = ${JSON.stringify(presets_1.THEME_PRESETS)};
