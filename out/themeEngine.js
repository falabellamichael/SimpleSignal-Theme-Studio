...
class ThemeEngine {
    ...
    static getEffectiveThemeState() {
        ...
        const themeName = workbench.get('colorTheme') || 'Default Dark+';
    ...
    static async applyTokenColors(colors) {
        ...
        for (const [syntaxId, rawColor] of Object.entries(colors)) {
            ...
            const targetScopes = presets_1.SYNTAX_SCOPE_MAP[syntaxId] || [syntaxId];
            const matchingIndices = updated.reduce((indices, rule, index) => {
                const scopes = Array.isArray(rule.scope) ? rule.scope : [rule.scope];
                if (scopes.some((scope) => targetScopes.includes(scope)))
                    indices.push(index);
                return indices;
            }, []);
